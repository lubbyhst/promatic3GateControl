...
public class BME280 {
    ...
    private static final Logger logger = Logger.getLogger(BME280.class.getName());
    ...
    public BME280Result pollBME280() throws IOException, I2CFactory.UnsupportedBusNumberException {
        if (isSimulated) {
            logger.fine("Simulated ENV. Returning random values.");
            return new BME280Result(new Random(System.currentTimeMillis()).nextDouble() * 100,
                    new Random(System.currentTimeMillis() + 100).nextDouble() * 100,
                    new Random(System.currentTimeMillis() + 100).nextDouble() * 100);
        }
        // Create I2C bus
        ...
        double var2 =
                ((((double) adc_t) / 131072.0 - ((double) dig_T1) / 8192.0) * (((double) adc_t) / 131072.0 - ((double) dig_T1) / 8192.0))
                        * ((double) dig_T3);
        final double t_fine = (long) (var1 + var2);
        final double cTemp = (var1 + var2) / 5120.0;
        final double fTemp = cTemp * 1.8 + 32;
        ...
        final double pressure = (p + (var1 + var2 + ((double) dig_P7)) / 16.0) / 100;

        // Humidity offset calculations
        double var_H = (((double) t_fine) - 76800.0);
        var_H = (adc_h - (dig_H4 * 64.0 + dig_H5 / 16384.0 * var_H)) * (dig_H2 / 65536.0 * (1.0 + dig_H6 / 67108864.0 * var_H * (1.0
